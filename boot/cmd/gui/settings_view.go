...
func (s *SettingsViewWidget) Build(context *guigui.Context, appender *guigui.ChildWidgetAppender) error {
	...
	if err != nil {
		...
		// For now, we'll proceed without the icon if it fails to load.
		// return err
	}
	...
	s.colorModeSegmentedControl.SetItems([]basicwidget.SegmentedControlItem[string]{
		...
		{
			Icon: lightModeImg, // This will be nil if image loading failed
			ID:   "light",
		},
		{
			Icon: darkModeImg, // This will be nil if image loading failed
			ID:   "dark",
		},
	})
